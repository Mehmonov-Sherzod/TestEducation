# 100 ta matematik savol qo'shish
$TopicId = "019b368e-b3ab-7212-8122-409d91a3befc"
...
$levelMap = @{ "easy" = 0; "medium" = 1; "hard" = 2 }
...
function Add-Question {
    ...
    for ($i = 0; $i -lt $Question.a.Count; $i++) {
        $answers += @{
            ...
            Translate = @(@{ LanguageId = 0; ColumnName = "Text"; TranslateText = $Question.a[$i] })
        }
    }
    ...
    $body = @{
        ...
        Translate = @(@{ LanguageId = 0; ColumnName = "QuestionText"; TranslateText = $Question.q })
        Answers = $answers
    }
    ...
    try {
        $response = Invoke-RestMethod -Uri "$ApiBase/api/QuestionAnswer" -Method Post -ContentType "application/json; charset=utf-8" -Body ([System.Text.Encoding]::UTF8.GetBytes($json))
...
Write-Host "100 ta matematik savol qo'shilmoqda..." -ForegroundColor Cyan
Write-Host "TopicId: $TopicId" -ForegroundColor Yellow

$success = 0; $errors = 0

Write-Host "`nEasy (30 ta)..." -ForegroundColor Green
for ($i = 0; $i -lt 30; $i++) {
    if (Add-Question -Question $mathQuestions.easy[$i] -Level "easy") { $success++ } else { $errors++ }
    Write-Host "`rEasy: $($i+1)/30" -NoNewline
}
Write-Host "`nEasy tugadi: $success" -ForegroundColor Green

$s1 = $success
Write-Host "`nMedium (40 ta)..." -ForegroundColor Yellow
for ($i = 0; $i -lt 40; $i++) {
    if (Add-Question -Question $mathQuestions.medium[$i] -Level "medium") { $success++ } else { $errors++ }
    Write-Host "`rMedium: $($i+1)/40" -NoNewline
}
Write-Host "`nMedium tugadi: $($success - $s1)" -ForegroundColor Yellow

$s2 = $success
Write-Host "`nHard (30 ta)..." -ForegroundColor Red
for ($i = 0; $i -lt 30; $i++) {
    if (Add-Question -Question $mathQuestions.hard[$i] -Level "hard") { $success++ } else { $errors++ }
    Write-Host "`rHard: $($i+1)/30" -NoNewline
}
Write-Host "`nHard tugadi: $($success - $s2)" -ForegroundColor Red

Write-Host "`n==============================" -ForegroundColor Cyan
Write-Host "Jami muvaffaqiyatli: $success/100" -ForegroundColor Green
Write-Host "Xatolar: $errors" -ForegroundColor Red
